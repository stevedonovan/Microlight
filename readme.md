...
`import` adds key/value pairs to a map, and `extend` appends a list to a list; they are two complementary ways to add multiple items to a table in a single operation.
...
`indexby` indexes a table by a list of keys:
...
Here is the old standby `imap`, which makes a _new_ list by applying a function to the original elements:
...
Finally, `collect` makes a list out of an iterator. The second argument can be the number of values to collect, which is useful for iterators that never terminate.
...
There is `icopy`, but note that `sub(t,1)` already does a simple list copy:
...
But we can add methods to the class directly if the functions have the right first argument, and don't return anything:
    ...
    ml.import(List,{
        ...
        filter=C(List,ml.ifilter),
        sub=C(List,ml.sub),
        indexby=C(List,ml.indexby),
    })
...
Lists are easier to use and involve less typing because the table functions are directly available from them. Methods may be _chained_, which (I think) reads better than the usual functional application order from right to left.
